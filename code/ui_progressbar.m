function ui_progressbar(fractiondone, position, caption)
%function progressbar(fractiondone, position)
%
% See also: https://www.mathworks.com/matlabcentral/fileexchange/6922-progressbar
...
% 2004-Oct-20   Efficient if-else structure for sec2timestr
    persistent progfig progpatch starttime lastupdate lastcaption
    ...
    if isempty(progfig)
        ...
        if (length(position) == 1)
            ...
            switch position
                ...
                otherwise
                    warning('OMEX:InvalidProgressbarPosition', 'Position must be (0-5). Reset to 0.')
            end
        ...
        elseif length(position) == 2
            ...
            if (position(1) < 0) || (scrsz(3)-width < position(1))
                ...
                warning('OMEX:InvalidProgressbarPosition', 'Horizontal position adjusted to fit on screen.')
            end
            if (position(2) < 0) || (scrsz(4)-height < position(2))
                position(2) = max(min(position(2),scrsz(4)-height),0);
                warning('OMEX:InvalidProgressbarPosition', 'Vertical position adjusted to fit on screen.')
            end
        else
        ...
        progfig = figure(...
            ...
            'BackingStore',     'off' );
        progaxes = axes(...
            'Position',         [0.02 0.15 0.96 helper],...
            'XLim',             [0 1],...
            'YLim',             [0 1],...
            'Box',              'on',...
            'ytick',            [],...
        ...
        if nargin > 2
            text(0.0,1.1,caption,'HorizontalAlignment','left','VerticalAlignment','bottom')
        end
    ...
    % Enforce a minimum time interval between updates but make sure this is
    % not the closing call
    if (percentdone < 100) && (etime(clock,lastupdate) < 0.01)
        return
    end
    ...
    if percentdone >= 100 
        % Task completed: close the progress bar
        delete(progfig) 
        % Clear persistent vars
        clear progfig progpatch starttime lastupdate lastcaption
        drawnow
    end
end
...
% set(progpatch,'FaceColor',thiscolor);
    set(progpatch,'FaceColor',[0.0392  0.1412 0.3922]);
end
