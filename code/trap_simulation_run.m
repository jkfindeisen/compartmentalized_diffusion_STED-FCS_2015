function [Ds, Di, TauD] = trap_simulation_run(Tp, Np, Dp, dt, SR, Trap, fwhms, seed)
% FCS simulation run for trapped diffusion.
%
% Simulates trajectories and photon traces, computes correlations, fits
% anomalous diffusion model.
%
% Syntax
%   function [Ds, Di, TauD] = trap_simulation_run(Tp, Np, Dp, dt, Trap, fwhms, seed)
%
% Input parameters
%   Tp - total time [s]
...
%   Trap - structure holding all trapping related parameters
%       Trap.N - number of trapping centers in the sandbox volume
%       Trap.D - Free diffusion coefficient of trap box center [m^2/s]
%       Trap.R - Trapping radius (around center) [m]
%       Trap.pon - probability for trapping (when distance < Trap.R)
%       Trap.poff - probability for releasing (when distance < Trap.R and trapped)
...
assert(nargin == 8, 'Wrong number of parameter!');
...
for kn = 1 : sim.N
    ...
    for ks = 1 : Nsim
        ...
        % get trajectory
        [x, y, t] = trap_simulation_trajectory(sim.D, sim.R, T, sim.dt, x0, y0, Trap);
        x0 = x(end);
        y0 = y(end);
        trajectory{ks} = [x, y];
    end
